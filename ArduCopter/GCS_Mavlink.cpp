...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_ATTITUDE: {
        ...
        const float x = mavlink_msg_attitude_get_roll(msg);
        const float y = mavlink_msg_attitude_get_pitch(msg);
        const float z = mavlink_msg_attitude_get_yaw(msg);
        copter.set_vicon_position(Vector3f(x, y, z));
        const float roll = mavlink_msg_attitude_get_rollspeed(msg);
        const float pitch = mavlink_msg_attitude_get_pitchspeed(msg);
        const float yaw = mavlink_msg_attitude_get_yawspeed(msg);
        copter.set_vicon_attitude(Vector3f(roll, pitch, yaw));

        copter.compass.setHIL(0, roll, pitch, yaw);
        break;
    }
